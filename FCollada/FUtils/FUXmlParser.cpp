...
namespace FUXmlParser
{
	...
	const fm::string& XmlToString(const char* s)
	{
...
#ifdef UNICODE
	// Convert a XML string to a text string: handles the '%' character
	const fstring& XmlToString(const fchar* s)
	{
		// Replace any '%' character string into the wanted characters: %20 is common.
		xmlBuilder.clear();
		fchar c;
	...
	const fm::string& ReadNodeProperty(xmlNode* node, const char* property)
	{
		if (node != NULL && property != NULL)
		{
			...
			if (data != NULL)
			{
				...
				const fm::string& ret = XmlToString((const char*) data);
				xmlFree(data);
				return ret;
			}
		}
	...
	const fm::string& ReadNodeContentFull(xmlNode* node)
	{
		if (node != NULL)
		{
			...
			{
				// Process the string for special characters
				const fm::string& ret = XmlToString((const char*) content);
				xmlFree(content);
				return ret;
			}
		}
