...
template<> const fstring& FUStringBuilder::ToString() const
{
	static fstring s;
	s = ToCharPtr();
	return s;
...
{
	fchar sz[128];
	fsnprintf(sz, 128, FC("%i"), i);
	append(sz);
}

#ifdef UNICODE
template<> const fm::string& FUSStringBuilder::ToString() const
{
	static fm::string s;
	s = ToCharPtr();
	return s;
