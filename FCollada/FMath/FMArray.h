...
namespace fm
{
	...
	*/
	...
	class vector
	{
#ifdef _DEBUG // This is protected, in DEBUG, for pvector::first.
	protected:
#else
	private:
#endif
	...
	public:
		...
		{
			insert(NULL, copy.begin(), copy.size());
		}
		...
		/** Return an pointer to our internal buffer.  Be very careful when using this. */
		inline T** GetDataPtr() const { return (T**)&heapBuffer; };
		...
		iterator erase(iterator it)
		{
			...
			memcpy(it, it+1, (end() - it - 1) * sizeof(T));
			--sized;
			return it;
		}
		...
		/** Inserts one item, multiple times at a given position within the list.
			...
			@param item The item to insert. */
		inline void insert(iterator it, size_t count, const T& item)
		{
			if (count > 0)
			{
		...
		inline T& operator[](size_t index) { FUAssert(index < sized, ); return heapBuffer[index]; } /**< See above. */
		inline const T& operator[](size_t index) const { FUAssert(index < sized, ); return heapBuffer[index]; } /**< See above. */
