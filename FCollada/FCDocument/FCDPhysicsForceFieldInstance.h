...
protected:
	...
	FCDPhysicsForceFieldInstance(FCDocument* document, FCDSceneNode* parent,
...
public:
	...
	/** [INTERNAL] Reads in the physics force field instance components from a 
		XML tree node.
		@param instanceNode The XML tree node.
		@return The status of the import. If the status is 'false', it may be 
			dangerous to use the physics force field instance. */
	virtual bool LoadFromXML(xmlNode* instanceNode);

	/** [INTERNAL] Writes out the \<instance_force_field\> element to the given
