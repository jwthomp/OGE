...
bool FCDEffectPassShader::LoadFromXML(xmlNode* shaderNode)
{
	bool status = true;
	if (!IsEquivalent(shaderNode->name, DAE_SHADER_ELEMENT))
	{
		FUError::Error(FUError::WARNING, FUError::WARNING_UNKNOWN_PASS_SHADER_ELEMENT, shaderNode->line);
		return status;
	}
	...
	if (name.empty())
	{
		FUError::Error(FUError::WARNING, FUError::WARNING_UNAMED_EFFECT_PASS_SHADER, shaderNode->line);
		return status;
	}
	fm::string stage = ReadNodeStage(shaderNode);
	isFragment = stage == DAE_FXCMN_FRAGMENT_SHADER;
	if (!isFragment && !IsEquivalent(stage, DAE_FXCMN_VERTEX_SHADER))
	{
		FUError::Error(FUError::WARNING, FUError::WARNING_UNKNOWN_EPS_STAGE, shaderNode->line);
		return status;
	}
