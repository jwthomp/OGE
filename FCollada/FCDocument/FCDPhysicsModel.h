...
public:
	...
	*/
	...
	/** [INTERNAL] Reads in the physics model components from a XML tree node.
		@param node The XML tree node.
		@return The status of the import. If the status is 'false', it may be 
			dangerous to use the physics model. */
	virtual bool LoadFromXML(xmlNode* node);

	/** [INTERNAL] Writes out the \<physics_model\> element to the given 
