...
bool FCDAnimated::LoadFromXML(xmlNode* parent)
{
	bool linked = false;
	// Write down the expected target string for the given node
	FUDaeParser::CalculateNodeTargetPointer(parent, pointer);
	...
	return ProcessChannels(channels) || linked; // [staylor]  This refactor mimics earlier functionality, but it this
}												// comparison here makes no sense?  Shouldnt we test for success on LinkDriver?
...
bool FCDAnimated::ProcessChannels(FCDAnimationChannelList& channels)
{
	...
	for (FCDAnimationChannelList::iterator it = channels.begin(); it != channels.end(); ++it)
	{
		...
		FCDAnimationCurveTrackList& channelCurves = channel->GetCurves();
		if (channelCurves.empty()) continue;
		
		// Retrieve the channel's qualifier and check for a requested matrix element
		fm::string qualifier = channel->GetTargetQualifier();
		if (arrayElement != -1)
		{
...
FCDAnimated* FCDAnimated::Clone(FCDocument* document, const float* animatedValue, FloatPtrList& newAnimatedValues)
{
	...
	for (size_t i = 0; i < toClone->curves.size(); ++i) 
	{
		...
		{
			FCDAnimationCurve* clonedCurve = const_cast<FCDAnimationCurve*>(toClone->curves[i][j])->GetParent()->AddCurve();
			toClone->curves[i][j]->Clone(clonedCurve);
			clone->AddCurve(i, clonedCurve);
		}
	}
