...
namespace FCDSceneNodeTools
{
	...
	void GenerateSampledAnimation(FCDSceneNode* node)
	{
		...
		for (FCDTransformContainer::iterator it = node->GetTransforms().begin(); it != node->GetTransforms().end(); ++it)
		{
			...
			if (animated != NULL)
			{
				...
				if (!(*it)->HasType(FCDTRotation::GetClassType())) angleIndices.push_back(-1);
