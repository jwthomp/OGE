...
public:
	...
	/** [INTERNAL] Reads in the physics material components from a XML tree 
		node.
		@param physicsMaterialNode The XML tree node.
		@return The status of the import. If the status is 'false', it may be 
			dangerous to use the physics material. */
	virtual bool LoadFromXML(xmlNode* physicsMaterialNode);

	/** [INTERNAL] Writes out the \<physics_material\> element to the given 
