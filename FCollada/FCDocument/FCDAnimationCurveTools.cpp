...
namespace FCDAnimationCurveTools
{
	...
	FCDAnimationMultiCurve* MergeCurves(const FCDAnimationCurveConstList& toMerge, const FloatList& defaultValues, const StringList& defaultQualifiers)
	{
		size_t dimension = toMerge.size();
		if (dimension == 0) return NULL;

		// Look for the document pointer and select the master curve.
		...
		for (size_t i = 0; i < dimension; ++i)
		{
			...
			if (curve == NULL || curve->GetKeyCount() == 0)
			{
				...
				{
					multiCurve->SetTargetQualifier(i, defaultQualifiers[i]);
				}
				continue;
			}

			multiCurve->SetTargetQualifier(i, curve->GetTargetQualifier());
