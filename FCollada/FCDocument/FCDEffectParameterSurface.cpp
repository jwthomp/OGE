...
size_t FCDEffectParameterSurface::AddImage(FCDImage* image, size_t index)
{
	...
	if (exists == (size_t) -1)
	{
		...
		{
			FUAssert(index > images.size(), return (size_t) -1);
...
bool FCDEffectParameterSurface::LoadFromXML(xmlNode* parameterNode)
{
	...
	for (xmlNodeList::iterator it = valueNodes.begin(); it != valueNodes.end(); ++it)
	{
		...
		{
			names.clear();
			FUError::Error(FUError::WARNING, FUError::WARNING_EMPTY_INIT_FROM, parameterNode->line);
		}
	...
	if (!initialized)
	{
		...
		if (valueNode)
		{
			...
			if (refNode)
			{
				...
				if (name.empty())
				{
					FUError::Error(FUError::WARNING, FUError::WARNING_EMPTY_IMAGE_NAME, surfaceNode->line);
				}
	...
	size = FUStringConversion::ToVector3(ReadNodeContentDirect(sizeNode));
		...
		switch (initMethod->GetInitType())
		{
			case FCDEffectParameterSurfaceInitFactory::FROM:
			{
				...
				else
				{
					...
					for (const FCDImage** itV = images.begin(); itV != images.end(); ++itV) 
					{ 
			...
			case FCDEffectParameterSurfaceInitFactory::CUBE:
			{
				...
				if (in->cubeType == FCDEffectParameterSurfaceInitCube::FACE)
				{
					for (size_t i = 0; i < images.size(); i++)
					{
						...
						AddAttribute(faceNode, DAE_REF_ATTRIBUTE, images[i]->GetDaeId());
					}
				}
				break;
			}
