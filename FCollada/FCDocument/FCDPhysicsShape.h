...
public:
	...
	/** [INTERNAL] Reads in the physics shape components from a XML tree node.
		If the mass or the density is not given, then it will be derived from
		mass = density x volume. If neither mass nor density is defined, then
		density defaults to 1.0 and mass is calculated. These are simply 
		default values, so if both are defined, then they are read as is.
		@param node The XML tree node.
		@return The status of the import. If the status is 'false', it may be 
			dangerous to use the shape. */
	virtual bool LoadFromXML(xmlNode* node);

	/** [INTERNAL] Writes out the \<shape\> element to the given COLLADA XML
		tree node.
