...
bool FCDEffectParameterSampler::LoadFromXML(xmlNode* parameterNode)
{
	...
	if (samplerNode == NULL)
	{
		FUError::Error(FUError::WARNING, FUError::WARNING_SAMPLER_NODE_MISSING, parameterNode->line);
		return status;
	}
	...
	if (surfaceSid.empty())
	{
		FUError::Error(FUError::WARNING, FUError::WARNING_EMPTY_SURFACE_SOURCE, parameterNode->line);
		return status;
	}
...
xmlNode* FCDEffectParameterSampler::WriteToXML(xmlNode* parentNode) const
{
	...
	AddChild(samplerNode, DAE_SOURCE_ELEMENT, surface != NULL ? surface->GetReference() : "");
