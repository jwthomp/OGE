...
FCDEffectCode* FCDEffectProfileFX::FindCode(const fm::string& sid)
{
...
const FCDEffectCode* FCDEffectProfileFX::FindCode(const fm::string& sid) const
{
...
FCDEffectParameter* FCDEffectProfileFX::FindParameterBySemantic(const fm::string& semantic)
{
...
void FCDEffectProfileFX::FindParametersBySemantic(const fm::string& semantic, FCDEffectParameterList& _parameters)
{
...
void FCDEffectProfileFX::FindParametersByReference(const fm::string& reference, FCDEffectParameterList& _parameters)
{
	// Look in the local parameters
...
bool FCDEffectProfileFX::LoadFromXML(xmlNode* profileNode)
{
	bool status = FCDEffectProfile::LoadFromXML(profileNode);
	...
	for (xmlNode* child = profileNode->children; child != NULL; child = child->next)
	{
		...
		if (IsEquivalent(child->name, DAE_TECHNIQUE_ELEMENT))
		{
			...
			status &= (technique->LoadFromXML(child));
		}
...
xmlNode* FCDEffectProfileFX::WriteToXML(xmlNode* parentNode) const
{
	...
	for (FCDEffectCodeContainer::const_iterator itC = codes.begin(); itC != codes.end(); ++itC)
	{
		(*itC)->LetWriteToXML(profileNode);
	}

	// Write out the techniques
	for (FCDEffectTechniqueContainer::const_iterator itT = techniques.begin(); itT != techniques.end(); ++itT)
	{
		(*itT)->LetWriteToXML(profileNode);
	}
