...
bool FCDEffectParameter::LoadFromXML(xmlNode* parameterNode)
{
	bool status = true;
	...
	for (xmlNodeList::iterator itN = annotationNodes.begin(); itN != annotationNodes.end(); ++itN)
	{
		...
		for (xmlNode* valueNode = annotateNode->children; valueNode != NULL; valueNode = valueNode->next)
		{
			...
			else {FUError::Error(FUError::WARNING, FUError::WARNING_INVALID_ANNO_TYPE, valueNode->line);}
			break;
		}
	}
	...
	if (isGenerator)
	{
		...
		if (reference.empty())
		{
			FUError::Error(FUError::WARNING, FUError::WARNING_GEN_REF_ATTRIBUTE_MISSING, parameterNode->line);
			return status;
		}
	}
	else
	{
		...
		if (reference.empty())
		{
			FUError::Error(FUError::WARNING, FUError::WARNING_MOD_REF_ATTRIBUTE_MISSING, parameterNode->line);
			return status;
		}
	}
...
bool FCDEffectParameterInt::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_FLOAT_VALUE, parameterNode->line);
	}
...
bool FCDEffectParameterBool::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_BOOLEAN_VALUE, parameterNode->line);
	}
...
bool FCDEffectParameterString::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
...
bool FCDEffectParameterFloat::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_FLOAT_PARAM, parameterNode->line);
	}
...
bool FCDEffectParameterFloat2::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_FLOAT_PARAM2, parameterNode->line);
	}
...
bool FCDEffectParameterFloat3::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_FLOAT_PARAM3, parameterNode->line);
	}
	value = FUStringConversion::ToVector3(valueString);
...
{
	xmlNode* parameterNode = FCDEffectParameter::WriteToXML(parentNode);
	fm::string s = FUStringConversion::ToString(value);
...
bool FCDEffectParameterVector::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_FLOAT_PARAM4, parameterNode->line);
	}
...
bool FCDEffectParameterMatrix::LoadFromXML(xmlNode* parameterNode)
{
	bool status = FCDEffectParameter::LoadFromXML(parameterNode);
	...
	if (valueString == NULL || *valueString == 0)
	{
		...
		FUError::Error(FUError::ERROR, FUError::ERROR_BAD_MATRIX, parameterNode->line);
	}
	FUStringConversion::ToMatrix(valueString, matrix);
	SetDirtyFlag();
	return status;
}
...
{
	xmlNode* parameterNode = FCDEffectParameter::WriteToXML(parentNode);
	fm::string s = FUStringConversion::ToString(matrix); 
