...
void FCDAnimationCurve::WriteSourceToXML(xmlNode* parentNode, const fm::string& baseId) const
{
	...
	for (FCDAnimationKeyList::const_iterator it = keys.begin(); it != keys.end(); ++it)
	{
		...
		if (hasTangents)
		{
			...
			else if ((*it)->interpolation == FUDaeInterpolation::LINEAR)
			{
				...
				const FCDAnimationKey* previousKey = it == keys.begin() ? NULL : *(it - 1);
				const FCDAnimationKey* nextKey = (it + 1) == keys.end() ? NULL : *(it + 1);
				...
				float inIntervalY = previousKey == NULL ? 0.0f : (*it)->output - previousKey->output;
				float outIntervalY = nextKey == NULL ? 0.0f : nextKey->output - (*it)->output;
				inTangents.push_back(FMVector2((*it)->input - inIntervalX / 3.0f, (*it)->output - inIntervalY / 3.0f));
				outTangents.push_back(FMVector2((*it)->input + outIntervalX / 3.0f, (*it)->output + outIntervalY / 3.0f));
			}
			else
			{
				// Export flat tangents
				inTangents.push_back(FMVector2((*it)->input - 0.0001f, (*it)->output));
				outTangents.push_back(FMVector2((*it)->input + 0.0001f, (*it)->output));
			}
		}
		if (hasTCB)
		{
			...
			{
				FCDAnimationKeyTCB* tkey = (FCDAnimationKeyTCB*) (*it);
...
xmlNode* FCDAnimationCurve::WriteSamplerToXML(xmlNode* parentNode, const fm::string& baseId) const
{
...
xmlNode* FCDAnimationCurve::WriteChannelToXML(xmlNode* parentNode, const fm::string& baseId, const char* targetPointer) const
{
