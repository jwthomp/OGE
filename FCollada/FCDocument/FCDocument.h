...
{
public:
	uint8 major; /**< The major release number of the version. */
	uint8 minor; /**< The minor release number of the version. */
	uint8 revision; /**< The revision number of the release. */

	/** Constructor. */
	FCDVersion() : major(0), minor(0), revision(0) {}
	...
	/** Retrieves whether this version number is lower than the given version string.
		@param v A string containing a version number.
		@return Whether this version is lower than the given version number. */
	bool IsLowerThan(const fm::string& v) const;

	/** Retrieves whether this version number is higher than the given version string.
		@param v A string containing a version number.
		@return Whether this version is higher than the given version number. */
	bool IsHigherThan(const fm::string& v) const;

	/** Retrieves whether this version number is equal to the given version string.
		@param v A string containing a version number.
		@return Whether this version is equal to the given version number. */
...
public:
	...
	/** [INTERNAL] Writes out the document to a XML node tree. This is the first step of the document
		...
		the given XML node tree. It also updates the sub ids so they do not conflict.
		@param colladaNode The base XML node tree to write to.
		@return The status of the import. If the status is 'false', it may be dangerous to
			extract information from the document. */
	bool WriteDocumentToXML(xmlNode* colladaNode);
