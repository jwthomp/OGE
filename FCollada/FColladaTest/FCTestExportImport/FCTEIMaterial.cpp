...
namespace FCTestExportImport
{
	...
	bool CheckEffectFX(FULogFile& fileOut, FCDEffectProfileFX* profile)
	{
		...
		PassIf(profile->GetParameters()->GetParameterCount() == 3);
		...
		{
			FCDEffectParameter* p = profile->GetParameters()->GetParameters().at(i);
		...
		PassIf(technique->GetParameterList()->GetParameterCount() == 1);
		FCDEffectParameter* float2Parameter = *technique->GetParameterList()->begin();
